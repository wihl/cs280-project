...
Because we choose to represent musical structure in terms of the surprise dynamics of the listener, it is necessary to describe the way in which the listener's distribution over future events changes as they hear present events. In the running example, by hearing $X="A-Major"$ in the present, how does their distribution over future events differ from how it was before they heard that chord. It is necessary to describe this ``difference" between the distributions. The Kullback-Leibler Divergence of one distribution from another captures this notion of distance. Let $X'$ be the revised distribution
$$ D_{KL}(X'\textrm{ }|| \textrm{ } X) =  \sum_{x \in \mathcal{X}} p_{X'}(x) log\frac{p_{X'}(x) }{p_{X}(x)}$$

\noindent This is read as "the divergence from X' of X". For an accessible yet informative discussion of the significance of Entropy as a measure of information and KL Divergence, see [insert link to Colah blog here]. This divergence describes the amount of revision to a listener's distribution over the future that happens as they hear each event. Let this be called the \textit{predictive information} of the event $X=x$ as the listener hears it. When a surprising event occurs and causes the listener to drastically revise their distribution (notably, this same event will be less surprising in the future), the event had high predicative information. On the other hand, if $30$ strikes of the same chord have just happened, hearing a $31^{st}$ articulation does not communicate much predictive information. In our system, the predictive information is measured as a rate over the duration of the piece (or work-in-progress), and is used to summarize the structure of a piece of music as it is expected to be perceived by the listener. Note that in the running example, entropy and divergence were discussed in terms of a sequence of chords heard by the listener, and the expectation over next chords in context. In even a simple piece of music, the listener tracks multiple such parameters and their interactions (see section Future Work): evolving harmony, rhythm, timbre, and more.

\subsection{Current Design}

Where we describe how composers receive feedback and interact with the automatic analysis.
Comparison of curves of characteristic piece to work-in-progress.
