...
SharedPlans \hl{[Grosz, Hunsberger]} define a robust framework for multi-agent, multi-level collaboration. A SharedPlan consists of a set of intentions, mutual beliefs, constraints, and hierarchies of actions and plans. All of these elements are applicable in this context, although only a subset of the general definition is needed for music composition purposes. 
...
Intentions are expressed by a minimum of two criteria: genre and mood. Genre and mood are algorithmically combined using existing methods \hl{[TODO: specify forward reference]} to define the approximate representation of the final work.
...
We intend to validate our system with several studies on small groups of composers that specify a musical goal and collaboratively work on piece of music facilitated by our system (Use Case 2). We also intend to validate our system for Use Case 1, \hl{in which} a non-musician requester feels that their desire has been addressed in some way. The Therapy use cases should be included in studies once these two less-complex cases are assessed.
...
Harmonia is intended to serve as a base platform for multiple aspects of musical creativity, while maintaining a sufficiently modular design to evolve. Specifically, the area of creative agents is undergoing rapid evolution. Providing a framework to ensure shared intentionality, artistic consistency and collaboration with humans should provide further acceleration. Multi-agent online improvisation such as AI Beats \hl{[TODO: What is "AI Beats"? Is it a specific thing needing a reference?]} is another ripe area for exploration both for artistic purposes as well as for `call and response' treatment of autism and other disorders. 
...
%4) MIDI AND XML same music to illustrate difference? (this is not a priority)

%5) % David Greenberg's point about Spotify and EchoNest. Might be important but also trying to reduce related work section at this point. I will think about it.

%------------------ David TODOs --------------------------------

%1) Add in some missing bibliography items, like Grosz, Hunsberger etc.

%2) NEW: What is "AI Beats"? Is it a specific thing needing a reference? I added this question in-line as well.

%------------------ GENERAL TODOs --------------------------------

%1) Fix the bibliography and in-text citations

%2) Fix the intra-paper references and section label references

%3) Another pass or two at spelling and grammar, text revision, clean read through

%4) How much of the formatting latex comments do we want to get rid of? Probably most of them, when we are almost done with the paper. Removing them may help us more easily notice what still needs to be done.

%5) Add these references to bib file:
