...
We intend to validate the system with several studies on small groups of composers that collaborate over completion of a musical goal. We also intend to validate for non-musician requesters seeking for their musical desires to be addressed. Therapy use cases should be included in studies once these two less-complex cases are assessed. Testing will include the validation of several system aspects:

\begin{enumerate}

\item Do composers understand and benefit from system feedback? If the composer proposes edits $A$ and $B$, and the system scores $B$ higher than $A$, which does the composer pick? What percentage of system-preferred edits do composers stick with? For system-suggested edits, what percentage of suggestions do composers accept?
...
To address point $1$, we will compare composer experience using a) the revision and metadata system without automated analysis, b) the system with score feedback for composer-specified edits, but no system-suggested edits, c) the system with score feedback for composer-specified edits and system suggestions. To address point $2$, we will explore other methods of inter-composer communication in place of the subplan issue/release procedure. Two approaches are a) traditional email to complement version-controlled music with no subplan options b) additional text documents to describe the equivalent of subplans but external to the SharedPlan and in longer form, committed along with the SharedPlan and musical score. Point $3$ will come from discussion with the requesters and composers.
...
This project was truly a collaborative effort between Mark and David. The ideas and effort could not have been more equally shared. Given Mark's music composition expertise, he contributed most of Section 4, ``Automated Analysis of Musical Structure.", and the related work on Collaborative Ideation. David contributed most of Section 2, "System Design" except for 2.4 ``MIDI and Edit Actions". All other sections are joint creations. We have the extensive git revision history to prove it!
